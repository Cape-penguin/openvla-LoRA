...
# 2. 저장했던 65MB짜리 LoRA 어댑터를 그 위에 얹습니다.
...
with torch.no_grad():
    for i, batch in enumerate(tqdm(eval_dataloader)):
        ...
        # 정답(Label) 추출: -100이 아닌 값(실제 액션 토큰)만 필터링
        # [BS, Seq_Len] 형태이므로 실제 토큰 ID만 뽑아냄
        gt_token_ids = batch["labels"][batch["labels"] != -100].tolist()
        
        # 모델 예측 (predict_action 사용 - 내부적으로 토큰 생성 및 디코딩 수행)
        # predict_action은 단일 샘플 처리에 최적화되어 있으므로 [0]번째 인덱스 참조 방식 활용
        ...
        # 정답 토큰 ID를 다시 실수(float) 액션값으로 복원 (비교용)
        # OpenVLA 공식 수식: (token_id - 31000) / 255 * 2.0 - 1.0
